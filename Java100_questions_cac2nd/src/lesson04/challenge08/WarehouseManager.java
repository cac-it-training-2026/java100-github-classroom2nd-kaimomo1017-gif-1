...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		int changeTimes = 0;
		do {
			for (int i = evenNumIndex; i < ABKosanArray1.length; i++) {
				if (ABKosanArray1[i] % 2 == 0) {
					evenNumIndex = i;
					break;
				}
			}
			for (int i = oddNumIndex; i < ABKosanArray2.length; i++) {
				if (ABKosanArray2[i] % 2 != 0) {
					int temp = ABKosanArray1[evenNumIndex];
					ABKosanArray1[evenNumIndex] = ABKosanArray2[i];
